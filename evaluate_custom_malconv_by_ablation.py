...
from secml_malware.models.my_dataloader import MyDataSet
from torch.utils.data import Dataset, DataLoader, random_split, default_collate

inp_len = 2 ** 21


def main(mal_path, ben_path, dir_path, dataset_size, total_ablations, batch_size=16):
    """
    ...
    for i, net in enumerate(nets):
        ...
        dataset = MyDataSet(mal_path, ben_path, inp_len, ablation_idx, total_ablations, dataset_size)
    ...
    votes = get_majority_voting_without_padding(np.asarray(train_preds_all_models), len(train_preds_all_models[0]),
                                                lengths_all_train,
                                                int(inp_len / total_ablations))
    train_acc = get_acc(votes, train_loader)
    print("Train Accuracy: ", train_acc)

    votes = get_majority_voting_without_padding(np.asarray(val_preds_all_models), len(val_preds_all_models[0]),
                                                lengths_all_val,
                                                int(inp_len / total_ablations))
    val_acc = get_acc(votes, valid_loader)
    print("Validation Accuracy: ", val_acc)

    votes = get_majority_voting_without_padding(np.asarray(test_preds_all_models), len(test_preds_all_models[0]),
                                                lengths_all,
                                                int(inp_len / total_ablations))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch_size', type=int, metavar='total_ablations', required=True)

    args = parser.parse_args()
    main(args.mal_path, args.ben_path, args.dir_path, args.dataset_size, args.ablations, args.batch_size)
